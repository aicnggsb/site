(function () {
    ...
    if (classInfoExportButton) {
        classInfoExportButton.addEventListener('click', () => {
            ...
            const buildT3ExportText = (moyT2, moyT3, t3b, t3t, t3a) => {
                ...
                const behaviorValues = [t3b, t3t, t3a].filter((value) => Number.isFinite(value));
                const safeTaux = behaviorValues.length ? clamp((behaviorValues.reduce((sum, value) => sum + value, 0) / behaviorValues.length) / 100, 0, 1) : 0.7;
            ...
            const exportRows = lastClassStudents.map((student) => {
                ...
                const moyenneComportement = Math.round(((effectiveB + effectiveT + effectiveA) / 3) * 10) / 10;
                const t3AppreciationCore = buildT3ExportText(
                    t2Value === '' ? NaN : Number(t2Value),
                    t3Value === '' ? NaN : Number(t3Value),
                    t3bValue === '' ? NaN : Number(t3bValue),
                    t3tValue === '' ? NaN : Number(t3tValue),
                    t3aValue === '' ? NaN : Number(t3aValue)
                );
                const formatHeaderValue = (value, suffix = '') => (Number.isFinite(value) ? `${value.toFixed(1)}${suffix}` : 'N/A');
                const t3Header = `[T3B=${formatHeaderValue(Number(t3bValue), '%')} | T3T=${formatHeaderValue(Number(t3tValue), '%')} | T3A=${formatHeaderValue(Number(t3aValue), '%')} | T3=${formatHeaderValue(Number(t3Value), '/20')} | T2=${formatHeaderValue(Number(t2Value), '/20')} | moyenne_comportement=${formatHeaderValue(moyenneComportement, '%')}]`;
                const t3Appreciation = `${t3Header} ${t3AppreciationCore}`;
