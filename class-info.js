(function () {
    ...
    function parseStudentData(row, idxMap) {
        ...
        return {
            ...
            cclasseT1: idxMap.classCommentT1Idx === -1 ? '' : (row[idxMap.classCommentT1Idx] || '').trim(),
            cclasseT2: idxMap.classCommentT2Idx === -1 ? '' : (row[idxMap.classCommentT2Idx] || '').trim(),
            cclasseT3: idxMap.classCommentT3Idx === -1 ? '' : (row[idxMap.classCommentT3Idx] || '').trim(),
        };
    }
...
<body class="project-evaluation-tab-body">
    ...
    async function fetchClassData(selectedClass, classConfig) {
        ...
        const classCommentT1Idx = resolveFirstExistingColumn(header, ['cclasse t1', 'cclasset1'], 25);
        const classCommentT2Idx = resolveFirstExistingColumn(header, ['cclasse t2', 'cclasset2'], 26);
        const classCommentT3Idx = resolveFirstExistingColumn(header, ['cclasse t3', 'cclasset3'], 27);
    ...
    if (classInfoExportButton) {
        classInfoExportButton.addEventListener('click', () => {
            ...
            const buildClassCommentList = (termKey) => {
                const comments = [...new Set(lastClassStudents.map((student) => (student[termKey] || '').trim()).filter(Boolean))];
                return comments.length
                    ? `<ul>${comments.map((comment) => `<li>${escapeSummaryHtml(comment)}</li>`).join('')}</ul>`
                    : '<p class="empty-summary-list">Aucune remarque classe renseignée.</p>';
            };
            const classCommentsHtml = [
                { label: 'Trimestre 1', key: 'cclasseT1' },
                { label: 'Trimestre 2', key: 'cclasseT2' },
                { label: 'Trimestre 3', key: 'cclasseT3' },
            ].map((term) => `<article class="class-comment-card"><h6>${term.label}</h6>${buildClassCommentList(term.key)}</article>`).join('');
