(function () {
    ...
    function buildReportHtml(report) {
        ...
        const studentCards = report.rows.map((row, studentIndex) => {
            ...
            const detailsId = `student-details-${studentIndex}`;
            return `<article class="student-card"><div class="student-header"><h2>${escapeHtml(row[report.nameIndex])}</h2><button type="button" class="student-toggle" aria-expanded="false" aria-controls="${detailsId}">Agrandir</button></div><div id="${detailsId}" class="terms-grid student-details">${termsHtml}</div></article>`;
        }).join('');

        return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Suivi PP</title><style>
            :root{--bg:#eef2f7;--panel:#fff;--text:#172033;--muted:#64748b;--border:#dbe3ee;--accent:#2563eb}*{box-sizing:border-box}body{margin:0;padding:24px;font-family:Arial,sans-serif;background:var(--bg);color:var(--text)}.topbar{max-width:1400px;margin:0 auto 18px;display:flex;justify-content:space-between;gap:16px;align-items:flex-start}.topbar h1{margin:0 0 5px}.topbar p{margin:0;color:var(--muted)}button{border:0;border-radius:9px;background:#334155;color:#fff;padding:9px 14px;font-weight:700;cursor:pointer}.class-summary{max-width:1400px;margin:0 auto 18px;padding:14px 18px;border-radius:14px;background:linear-gradient(135deg,#1d4ed8,#4f46e5);color:#fff;display:flex;gap:24px;flex-wrap:wrap}.class-summary strong{font-size:1.45rem}.students{max-width:1400px;margin:auto;display:grid;gap:10px}.student-card{break-inside:avoid;background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:12px 14px;box-shadow:0 5px 16px rgba(15,23,42,.06)}.student-header{display:flex;align-items:center;justify-content:space-between;gap:14px}.student-header h2{margin:0;font-size:1.1rem}.student-toggle{flex:none;background:var(--accent);padding:7px 12px}.student-toggle:hover,.student-toggle:focus-visible{background:#1d4ed8}.student-details{display:none;margin-top:14px}.student-card.is-expanded .student-details{display:grid}.terms-grid{grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:14px}.term-card{border:1px solid var(--border);border-radius:12px;overflow:hidden}.term-header{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px 14px;background:#f8fafc}.term-header h3{margin:0;font-size:1rem}.term-summary{display:flex;align-items:center;gap:10px;color:var(--muted)}.term-summary strong{color:var(--grade-color);font-size:1.15rem}.radar-wrap{padding:10px 12px 14px}.average-radar{display:block;width:100%;height:auto;max-height:430px}.radar-ring{fill:none;stroke:#dbe3ee;stroke-width:1}.radar-axis{stroke:var(--axis-color);stroke-width:1;stroke-opacity:.35}.radar-area{fill:rgba(37,99,235,.18);stroke:none}.radar-line{fill:none;stroke:#2563eb;stroke-width:2.5;stroke-linejoin:round}.radar-dot{fill:var(--dot-color);stroke:#fff;stroke-width:2}.radar-label{fill:var(--axis-color);font-size:12px;font-weight:700}.radar-scale{fill:#94a3b8;font-size:10px}.radar-legend{display:flex;justify-content:center;gap:14px;flex-wrap:wrap;color:var(--muted);font-size:.78rem;font-weight:700}.radar-legend span{display:inline-flex;align-items:center;gap:5px}.radar-legend i{width:9px;height:9px;border-radius:50%;background:var(--legend-color)}.radar-note{margin:8px 0 0;text-align:center;color:var(--muted);font-size:.75rem}.error{max-width:720px;margin:80px auto;background:#fff;border:1px solid #fecaca;border-radius:14px;padding:22px;color:#991b1b}@media print{body{padding:0;background:#fff}.topbar button,.student-toggle{display:none}.student-details,.student-card.is-expanded .student-details{display:grid}.student-card{box-shadow:none;page-break-inside:avoid}.class-summary{background:#fff;color:#172033;border:1px solid var(--border)}}@media(max-width:650px){body{padding:12px}.topbar{display:block}.topbar button{margin-top:12px}.terms-grid{grid-template-columns:1fr}.radar-label{font-size:11px}}
        </style></head><body><header class="topbar"><div><h1>Suivi PP</h1><p>Bilan élève par élève à partir des trimestres disponibles dans le CSV.</p></div><button type="button" onclick="window.print()">Imprimer / PDF</button></header><section class="class-summary"><span><strong>${report.rows.length}</strong><br>élèves</span><span><strong>${latestTerm ? `T${latestTerm.number}` : '—'}</strong><br>dernier trimestre disponible</span><span><strong>${classAverage === null ? 'N/A' : formatGrade(classAverage)}</strong><br>moyenne de classe</span></section><main class="students">${studentCards}</main><script>document.querySelector('.students').addEventListener('click',function(event){const button=event.target.closest('.student-toggle');if(!button)return;const card=button.closest('.student-card');const expanded=card.classList.toggle('is-expanded');button.setAttribute('aria-expanded',String(expanded));button.textContent=expanded?'Réduire':'Agrandir';});<\/script></body></html>`;
    }
    ...
    followupButton.addEventListener('click', async () => {
        ...
        try {
            ...
            viewWindow.document.open();
            viewWindow.document.write(buildReportHtml(report));
            viewWindow.document.close();
        } catch (error) {
